...
不建议在未配置认证、HTTPS、反向代理保护、访问控制和网络层限制的情况下开放公网访问。当前版本不提供管理员登录能力、不提供管理端页面访问控制，也不提供可作为公网安全方案的完整 OpenAPI Token 与权限模型。

后续 v1.1.x 将逐步补充管理员账号初始化、登录 / 登出、管理端页面访问控制、后端管理接口登录保护、OpenAPI Token、只读 Token 与写操作权限区分预留、反向代理部署建议、HTTPS / 公网暴露风险说明和 Docker Compose 安全部署示例。当前说明为安全边界提示，不代表相关能力已经实现。
