...
v1.2.1 已完成歌曲工作台 MVP：管理端可从导航或歌曲列表进入工作台，边播放当前歌曲，边查看数据库元数据、歌词、封面和 OpenAPI 输出预览。该版本只读校验，不新增确认状态、AI 候选、网络刮削、批量整理或文件写回。
...
- v1.2.2：在工作台基础上设计元数据 / 歌词 / 封面确认状态与只读校验后的人工闭环
