...
v0.9.7 补充了镜像拉取部署与手动发布准备文档：

- [Docker 镜像部署（拉取已发布镜像）](docs/deployment/image-deploy.md)
- [源码构建部署（本地构建镜像）](docs/deployment/docker.md)
- [镜像发布说明（GHCR / Docker Hub 手动流程）](docs/release/image-publish.md)
...
项目处于 **早期开发阶段**，后端基础能力已覆盖 v0.9.7（镜像发布与 Packages 准备），前端已推进至 v0.8.4，核心功能陆续实现中。
