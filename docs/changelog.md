...
- **统一歌词状态口径**：新增服务端统一状态快照，管理首页、歌曲列表和歌曲工作台消费同一套 `SWLRC_READY`、`LRC_READY`、`NO_LYRICS`、`ALIGNMENT_RUNNING`、`DRAFT_PENDING`、`FAILED` 状态。运行中任务和待确认草稿优先显示；历史绑定存在但歌词源文件或 SWLRC 文件不可用时不再显示为已有歌词。
- **首页歌词资产进度**：新增“歌词资产进度”区域，展示可用歌曲总数、有可用歌词数、歌词覆盖率、SWLRC 覆盖率、仅有 LRC、无歌词、制作中和待确认数量，并提供“查看缺少 SWLRC”和“进入歌词工作台”入口。
- **每日稳定推荐**：首页每天生成并持久化最多 5 首缺少 SWLRC 的歌曲推荐；同一天刷新保持稳定，推荐区分 `LRC_UPGRADE` 和 `NO_LYRICS`，不会把已有 LRC 的歌曲误写成“无歌词”。
- **推荐操作**：推荐卡支持“开始制作”“今天跳过”“换一首”。开始制作只跳转现有歌词工作台，不绕过既有草稿 / 对齐工作流；跳过仅当天隐藏；换一首会保留原推荐记录并持久化补位结果。
- **随机候选**：新增随机挑选待制作歌曲，支持 5 / 10 / 20 首，前后端均限制最大 20；只生成候选列表，不批量创建任务。
- **歌曲列表扩展**：全部歌曲列表显示细分歌词状态 Badge，支持按缺少 SWLRC、已有 SWLRC、仅有 LRC、无歌词、制作中、待确认和制作异常筛选，筛选语义由后端统一计算。
- **数据库迁移**：新增 `lyric_daily_recommendation` 表，记录每日推荐槽位、推荐类型、操作状态、替换关系和操作时间。

### 不包含

- 不做 AI 智能推荐、播放行为推荐、网络歌词抓取、自动批量制作歌词、永久忽略歌曲、整库一键生成歌词或新的歌词编辑器。

## v1.3.2 — 歌词工作台体验优化

### 新增 / 优化

- **手工创建歌词草稿**：歌曲工作台支持跳过 Worker 草稿提取，直接粘贴歌词文本创建可编辑草稿。手工草稿使用 `LYRIC_DRAFT_MANUAL` 任务类型和 `MANUAL_PASTE` 来源语义，不伪装为 Worker 提取结果；后续仍走保存草稿、确认可信歌词、创建逐字对齐任务的既有链路。
- **Brave Search 候选来源辅助**：新增管理端 Brave Search 配置、状态查询、测试连接和搜索代理接口；草稿页可按歌曲信息搜索候选歌词来源，展示标题、域名、摘要和来源链接，并可把来源元信息关联到当前草稿。
- **Brave Key 双配置模式与安全边界**：支持 `MUSIC_VAULT_BRAVE_SEARCH_API_KEY` 环境变量模式和控制台托管模式。控制台托管 Key 使用 `MUSIC_VAULT_SETTINGS_ENCRYPTION_KEY` 服务端加密保存，不回显完整 Key；环境变量优先于控制台配置，ENV 模式下控制台保存、启用和暂停不会改变实际搜索 Key。
- **外部歌词来源边界收口**：Brave Search 只返回候选来源信息，不抓取、不下载、不缓存第三方网页正文，不把搜索摘要写入歌词正文。用户仍需自行打开来源页面、判断可信度并手工复制歌词到草稿编辑区。
- **管理端 SWLRC 逐字歌词试听**：歌曲工作台在存在 SWLRC 时优先使用逐字歌词渲染，支持按播放器进度逐字高亮；无 SWLRC、文件缺失、读取失败或解析失败时自动回退到原有 LRC 行级歌词。
- **歌词工作台布局优化**：草稿核心操作上移，编辑区保持稳定高度；质量摘要、质量报告、Worker 状态摘要补充中文说明，完整质量报告和低价值 Worker 原始细节后置，减少首屏占用。

### 兼容性

- 保留原有 Worker 草稿提取、可信歌词确认、逐字对齐、审核和导入流程。
- 保留现有任务状态轮询，不替换为 SSE，不引入文件监听。
- 保留 OpenAPI 与音乐盒读取链路；管理端 SWLRC 渲染只影响工作台试听体验。

### 不包含

- 不实现 Worker `progress.json`。
- 不展示真实 Worker 进度，也不伪造、估算或模拟百分比进度。
- 不实现 SSE、文件监听或轮询替换。
- 不自动抓取第三方歌词网页正文。
- 不实现逐字时间轴编辑器或批量全库对齐。
