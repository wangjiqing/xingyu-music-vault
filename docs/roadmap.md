...
## 当前阶段：v1.2.x — 歌曲工作台与边听边整理体验

> 阶段目标：在 v1.1.x 安全部署基础上，增强管理端歌曲校验、边听边看和后续整理闭环。

v1.2.1 已完成歌曲工作台 MVP：管理端可从导航或歌曲列表进入工作台，边播放当前歌曲，边查看数据库元数据、歌词、封面和 OpenAPI 输出预览。v1.2.2 已完成工作台小屏与低高度布局修复，播放器、歌词、封面、元数据和 OpenAPI 输出在窄内容区下可以缩放或滚动。当前 v1.2.x 仍保持只读校验，不新增确认状态、AI 候选、网络刮削、批量整理或文件写回。

v1.2.x 仍限定个人自托管、本地 / 局域网可信使用为主；如需公网访问，继续沿用 v1.1.x 的 HTTPS 反向代理、管理端登录和 OpenAPI HMAC 安全边界。

## v1.2.x — 后续方向

优先方向：

- 在工作台基础上设计元数据 / 歌词 / 封面确认状态与只读校验后的人工闭环
