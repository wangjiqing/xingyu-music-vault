...
## 当前阶段：v1.3.2 — 歌词工作台体验优化已完成

> 阶段目标：在 v1.3.0 歌词草稿与逐字对齐任务闭环、v1.3.1 正式对齐资产目录收口基础上，降低歌词整理、来源查找和逐字歌词试听的操作成本。

v1.3.2 已完成歌词工作台体验优化：支持跳过 Worker 草稿提取直接手工粘贴歌词创建草稿；新增 Brave Search 候选来源辅助和加密 Key 托管；管理端工作台可读取 SWLRC 并逐字试听，无 SWLRC 或解析异常时回退 LRC；草稿编辑区、质量摘要、质量报告和 Worker 状态摘要完成布局与可读性优化。

v1.3.2 仍限定个人自托管、本地 / 局域网可信使用为主；如需公网访问，继续沿用 HTTPS 反向代理、管理端登录和 OpenAPI HMAC 安全边界。Brave Search 仅用于候选来源发现，不抓取、不下载、不缓存第三方歌词网页全文。

## v1.3.3 / v1.4.0 — 后续收口方向
