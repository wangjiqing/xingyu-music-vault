...
## 当前阶段：v1.3.4 — 歌词任务可观测性接入

> 阶段目标：在既有歌词草稿、逐字对齐任务和歌词工作台基础上，兼容 `xingyu-lyrics-aligner v0.5.0` Worker 可观测协议，并接入 v0.6.0 歌词头部展示协议。

v1.3.4 已接入新版 `status.json` / `events.jsonl` 后端读取、任务可观测详情接口、列表/详情轻量摘要、任务详情 Worker 可观测性 tab、工作台轻量阶段/心跳展示，以及草稿提取 request schemaVersion 3 preset 表单。

当前仍限定个人自托管、本地 / 局域网可信使用为主；如需公网访问，继续沿用 HTTPS 反向代理、管理端登录和 OpenAPI HMAC 安全边界。Brave Search 仅用于候选来源发现，不抓取、不下载、不缓存第三方歌词网页全文。

## v1.3.4 / v1.4.0 — 后续收口方向

优先方向：

- 持续对接 `xingyu-lyrics-aligner` Worker 可观测协议：Worker 输出真实阶段、心跳、事件和结果摘要，音库读取真实状态。
- 评估 SSE 或等价实时状态推送，作为现有轮询之上的后续增强；不显示伪进度，不估算百分比。
...
> 阶段目标：接入 Worker v0.5.0 可观测协议，先完成后端兼容和接口，再完成管理端展示。

- [x] 后端读取 `status.json` 的阶段、心跳、attempt、配置、警告、错误和结果摘要
- [x] 后端读取最近 `events.jsonl` 事件，不入库、不做搜索
- [x] 新增任务可观测详情接口
- [x] 草稿提取任务 request 升级到 schemaVersion 3，并支持 preset
- [x] 管理端任务列表展示可观测摘要
- [x] 管理端详情 tab / 面板展示 status 与 events
- [x] 草稿创建表单提供 preset UI，并将高级参数默认折叠
- [x] 对齐任务完成后支持试听任务 LRC / SWLRC，并随原曲检查歌词滚动
- [x] 消费 Worker v0.6.0 非演唱头部与前奏 display-only 提示，不在音库侧重新识别或生成伪时间轴
- [x] 保持对无进度 Worker 的兼容降级：只展示已有真实任务状态
- [ ] 评估 SSE 或等价实时状态推送，作为现有轮询之上的后续增强

**边界**：不显示伪进度，不用文件监听替代协议，不把缺失进度包装成百分比；v1.3.4 不做日志搜索、日志分页、任务重试或多 Worker 调度。
