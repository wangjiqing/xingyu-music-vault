...
| `source_type` | 来源类型；本地扫描为 `LOCAL_FILE`，对齐导入为 `ALIGNMENT` |
...
| `source_task_id` | 对齐导入来源任务 ID；仅 `ALIGNMENT` 来源写入 |
...
| `sourceType` | `LOCAL_FILE` / `ALIGNMENT` / `MANUAL` / `ONLINE` | 当前本地扫描写入 `LOCAL_FILE`，歌词对齐导入写入 `ALIGNMENT`，其余为后续预留 |
