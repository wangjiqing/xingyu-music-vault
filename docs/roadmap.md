...
## 当前阶段：v1.3.0 — 歌词草稿与逐字对齐任务闭环已完成

> 阶段目标：在既有歌曲工作台基础上，完成“音频生成候选歌词草稿、人工校对可信歌词、可信歌词创建逐字对齐任务、LRC / SWLRC 审核导入”的管理端人工辅助链路。

v1.3.0 已完成歌词草稿提取、边听边校对、草稿确认可信歌词、可信歌词创建逐字对齐任务、Worker 共享目录同步、LRC / SWLRC 人工审核与确认导入。对齐和草稿提取由独立 `xingyu-lyrics-aligner:0.4.0` Worker 容器完成，音库容器仍只负责任务目录、数据库状态、人工审核和受控资产导入。

v1.3.0 仍限定个人自托管、本地 / 局域网可信使用为主；如需公网访问，继续沿用 HTTPS 反向代理、管理端登录和 OpenAPI HMAC 安全边界。

## v1.3.1 / v1.4.0 — 后续收口方向

优先方向：

- 正式对齐歌词资产目录收口：当前 `ALIGNMENT` LRC / SWLRC 已从 Worker jobs 目录复制到 `alignment-assets-dir`；后续应迁移至 `LYRICS_DIR` / `MUSIC_VAULT_LYRIC_DIRS` 下受控 `alignment` 子目录
- 扫描器需识别受控对齐资产目录，避免把 `ALIGNMENT` 资产重复识别为 `LOCAL_FILE`
- 删除同步需区分外部本地扫描歌词与音库生成歌词，避免误解绑或误删
- 迁移已有正式对齐资产时，先复制文件、校验 hash，再更新数据库路径
