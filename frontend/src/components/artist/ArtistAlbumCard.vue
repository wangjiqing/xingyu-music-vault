...
import type { ArtistAlbumItem } from '../../api/music'
...
<template>
  <div class="album-card" @click="handleClick">
    <div class="album-card-avatar" :style="{ background: albumColor }">
      <span v-if="initial" class="album-initial">{{ initial }}</span>
      <el-icon v-else :size="28"><Collection /></el-icon>
    </div>
    <div class="album-card-body">
      <div class="album-name" :title="item.album">{{ item.album }}</div>
