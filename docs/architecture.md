...
| Music Workbench | 管理端歌曲工作台、受保护音频播放、只读校验聚合、OpenAPI 输出预览 | 已实现 MVP（v1.2.1），小屏布局优化（v1.2.2） |
...
| 歌曲工作台 | `/music/workbench` | 已实现 MVP（v1.2.1，只读校验），小屏布局优化（v1.2.2） |
...
本工作台不改变数据库结构，不增加确认状态，不写回音频 Tag，不做网络刮削或 AI 能力；后续版本可在此页面基础上设计元数据 / 歌词 / 封面确认状态。
