...
- `MusicMetadataSyncService`：核心同步逻辑，支持单曲和批量（最多 20 条）两种模式，`diffs()` 仅比较 title/artist/album，`applySnapshotToTrack()` 仅写入这三个字段
...
**v0.8.5 不包含：** 高级字段同步、封面写入音频文件、网络/AI 刮削元数据自动同步、整库同步、按歌手一键全量同步。后续版本可考虑在 Tag 读取兼容性增强后引入高级字段同步。
