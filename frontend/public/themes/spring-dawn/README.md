...
- 源素材包内包含 `archive/original-previews/` 备份；公开静态目录未保留该目录，避免预览源图进入前端产物。

## 分辨率说明

- README banner 源裁切尺寸为 `1568x490`。`readme-banner@2x.png` 保留最佳可用尺寸，并非原生 3200x1000 2x。
- 背景源图为 `1672x941`，`isNative4K` 为 `false`。当前未做超分，旧 4K / 2K / 1080p 命名仅在 `manifest.json` 中作为 alias 记录，不是原生 4K。
...
- 源展示板缺少以下空状态单图，未伪造图片；已在 `manifest.json` 的 `emptyStatesMissing` 中标记为 `needs-redraw`: `syncing`, `metadataPending`。
