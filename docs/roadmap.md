...
## 当前阶段：v1.3.1 — 正式对齐歌词资产目录收口已完成

> 阶段目标：在 v1.3.0 歌词草稿与逐字对齐任务闭环基础上，将审核通过后的正式 `ALIGNMENT` LRC / SWLRC 收口到歌词目录下的受控 `alignment` 子目录，并隔离普通扫描与删除同步。

v1.3.1 已完成正式对齐歌词资产目录收口：新的审核导入会把 LRC / SWLRC 发布到显式配置的 `MUSIC_VAULT_ALIGNMENT_LYRICS_ROOT` 下受控 `alignment/{songId}/{jobId}` 子目录；普通歌词扫描会排除该目录，删除同步继续仅处理 `LOCAL_FILE`。旧 `alignment-assets-dir` 不自动迁移、不删除，历史 `ALIGNMENT` 资产继续按数据库原路径读取。

v1.3.1 仍限定个人自托管、本地 / 局域网可信使用为主；如需公网访问，继续沿用 HTTPS 反向代理、管理端登录和 OpenAPI HMAC 安全边界。

## v1.3.2 / v1.4.0 — 后续收口方向
